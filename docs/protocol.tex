...
streamlit run xcavate/gui/app.py
    ...
    \begin{enumerate}
        ...
        \item Generating collision-free print passes (DFS)
...
\gui{Pathfinding algorithm} & Selectbox & DFS & \textbf{DFS}: modified depth-first search with KD-tree collision detection. \\
...
\gui{Overlap algorithm} & Selectbox & Retrace & Only visible when overlap $> 0$. \textbf{Retrace}: scans all previous passes for shared nodes, retraces backwards, appends to end of current pass. \\
...
\code{--overlap\_algorithm} & str & retrace & \code{retrace}: scans all previous passes (original). \\
