...
\gui{Calibration Validation} & Upload calibration data to validate print accuracy against targets \\
\bottomrule
\end{tabularx}
...
\gui{Pathfinding algorithm} & select & DFS & DFS: thorough collision detection via depth-first search. Sweep Line: faster bottom-up approach for large networks. \\
...
wheel, and hover over traces for coordinate information.
...
\gui{Print speeds used during calibration} & text input & Comma-separated list of print speeds in mm/s, one per target diameter group (same order). Pressure is held constant; speed is varied for each calibration filament. \\
...
Reduce coordinates per vessel to 100 or fewer, or enable downsampling. The Sweep Line algorithm may also be faster for very large networks. \\
\midrule
