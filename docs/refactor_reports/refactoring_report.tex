...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{Correctness guarantee:} Numerically identical output on real data (\texttt{tree\_Test2.txt}) verified across all pipeline stages---both SM and MM---with default settings. Only cosmetic formatting differs (trailing whitespace, trailing decimal zeros).
...
No mathematical or algorithmic logic was altered---all changes are structural, organizational, and performance-related. The refactored code produces numerically identical output to the original for all tested inputs with default settings (cosmetic formatting differences only). An optional nearest-neighbor pass reordering feature (disabled by default) can be enabled to minimize nozzle travel; see Section~\ref{sec:equivalence-note} for details.
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Is the work produced equivalent to the original code?} Yes. With default settings, all output files are numerically and functionally identical to the original (cosmetic formatting differences only). An optional pass reordering feature (disabled by default) can be enabled to minimize nozzle travel distance. No mathematical or algorithmic logic was altered.
...
The fundamental equivalence guarantee holds: with default settings, all output files are numerically and functionally identical to the original (cosmetic formatting differences only). This has been verified through full-pipeline real-data comparison on \texttt{tree\_Test2.txt}, function-by-function unit testing, and 95 automated regression tests.
