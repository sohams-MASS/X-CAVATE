...
\textbf{With default settings, every output produced by the refactored X-CAVATE code is 100\% byte-identical to the output of the original monolithic script for the same input.}\\[0.5em]
This has been verified via byte-identical file comparison, function-level unit testing, and 95 automated regression tests. No mathematical or algorithmic logic was altered. An optional pass reordering feature (disabled by default) can be enabled to minimize nozzle travel distance.
}}
\end{center}
...
\subsubsection{Full Pipeline: Byte-Identical Output}

\begin{lstlisting}[language=bash,numbers=none,caption={All output files identical (default settings)}]
$ diff -rq outputs_baseline/ outputs_refactored/
# (no output - all files identical)
\end{lstlisting}

All output files---coordinates (SM and MM), changelog, graph, special nodes, and G-code---are byte-identical between the original and refactored versions when using default settings (\texttt{reorder\_passes=False}).
...
Output correctness & Reference & Byte-identical (default settings) \\
\bottomrule
\end{tabularx}
\end{table}
