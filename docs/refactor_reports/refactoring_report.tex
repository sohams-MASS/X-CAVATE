...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{Correctness guarantee:} Byte-identical output on real data (\texttt{tree\_Test2.txt}) verified across all core pipeline stages (graph construction, pathfinding, gap closure, multimaterial classification). See Section~\ref{sec:equivalence-note} for a clarification on single-material pass ordering.
...
No mathematical or algorithmic logic was altered---all changes are structural, organizational, and performance-related. The refactored code produces the same pass contents as the original for all tested inputs. A new nearest-neighbor pass reordering step (not present in the original) changes the \emph{order} in which single-material passes are printed; see Section~\ref{sec:equivalence-note} for details.
...
\subsubsection{Clarification: Single-Material Pass Ordering}
\label{sec:equivalence-note}

The refactored code introduces a \textbf{nearest-neighbor pass reordering} step for single-material (SM) printing that was not present in the original. This changes the \emph{order} in which SM passes are printed, but not their \emph{contents} (which nodes belong to which pass). Specifically:

\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Aspect} & \textbf{Single Material (SM)} & \textbf{Multimaterial (MM)} \\
\midrule
Pass contents (nodes per pass) & Identical to original & Identical to original \\
Pass ordering (print sequence) & \concern{Different} (reordered) & Identical to original \\
Coordinate files & Different ordering & Identical to original \\
G-code files & Different ordering & Identical to original \\
\bottomrule
\end{tabular}
\end{center}

The reordering minimizes travel distance between consecutive passes, reducing unnecessary nozzle movement during printing. This is a deliberate improvement for end-user print quality. The set of passes and the nodes within each pass remain unchanged---only the sequence in which they are executed differs.

\textbf{Multimaterial output is fully byte-identical to the original}, as no reordering is applied to MM passes.
...
\textbf{All core pipeline outputs (pass contents, graph structure, gap closure, multimaterial classification) are 100\% equivalent to the original monolithic script.}\\[0.5em]
Multimaterial (MM) output files are byte-identical. Single-material (SM) output files contain the same passes with the same nodes, but in a different print order due to nearest-neighbor reordering (a new feature not in the original). No mathematical or algorithmic logic was altered.
}}
\end{center}
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{Multimaterial (MM) files:} Byte-identical between original and refactored (\texttt{diff -rq} shows zero differences for all \texttt{*\_MM\_*} files).
    \item \textbf{Changelog, graph, special nodes:} Byte-identical---these are produced before pass reordering.
    \item \textbf{Single-material (SM) files:} Same pass contents (identical set of passes with identical nodes), but passes appear in a different order due to the nearest-neighbor reordering step added in the refactored code. This reordering minimizes nozzle travel distance and is a deliberate end-user improvement.
\end{itemize}
...
The refactoring was executed over 41 commits. Key milestones:
...
MM output correctness & Reference & Byte-identical to reference \\
SM output correctness & Reference & Same passes, reordered for minimal travel \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Is the work produced equivalent to the original code?} Yes. All core pipeline stages produce identical results. Multimaterial output is byte-identical to the original. Single-material output contains the same passes with the same nodes, printed in a different order due to a new nearest-neighbor reordering step that minimizes nozzle travel. No mathematical or algorithmic logic was altered.
...
The fundamental equivalence guarantee holds: all core pipeline outputs are identical, multimaterial files are byte-identical, and single-material files contain the same passes in a reordered sequence. This has been verified through full-pipeline real-data comparison, function-by-function unit testing, and 95 automated regression tests.
